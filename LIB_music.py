...
class DowloadError(DownloadError):
    """Ошибка скачивания"""


class YTDLError(ExtractorError):
    """Ошибка работы по созданию ресурса проигрывания"""
...
class Song:
    ...
    def create_embed(self):
        embed = (discord.Embed(title='Музыка',
                               description='```css\n{0.source.title}\n```'.format(self),
                               color=0xAF69EF)
            ...
            if not self.loop:
                ...
                if self.voice != None:
                    try:
                        ...
                        self.bot.loop.create_task(await self.stop())
                        return
            else:
                self.current.source = await YTDLSource.create_source(self._ctx, self.current.source.url, loop=self.bot.loop)
